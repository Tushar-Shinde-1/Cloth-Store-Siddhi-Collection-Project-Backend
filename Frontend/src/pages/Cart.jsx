...
const Cart = () => {
  ...
  const [cartData,setCartData]=useState([]);
    useEffect(()=>
    {
     ...
     // console.log("add to cart",tempData);
    },[cartItem]);
  
  
  return (
    <div className="border-t px-12 pt-14">
      ...
      cartData.map((item,index)=>
      {
        ...
        return(
          <div key={index} className='py-4  border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
            <div className='flex px-1 item-start gap-6'
            >
              <img src={
                productdata.image[0]
              }
              ...
              <img onClick={()=>updateQuantity(item._id,item.size,0)} src={assets.back} className='w-4 mr-4 sm:w-5 cursor-pointer' alt="" />
          </div>
        )
      })
    }
